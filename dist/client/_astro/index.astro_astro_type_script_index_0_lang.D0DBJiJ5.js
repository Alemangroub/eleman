import{c as v,l as f,g as u}from"./auth.nTrJDfvb.js";let l={id:null,name:""};async function h(){const t=document.getElementById("auth-loading-message"),e=document.getElementById("dashboard-content"),s=document.getElementById("user-name-placeholder");document.getElementById("projects-list"),document.getElementById("no-projects-message");const n=document.getElementById("logout-button"),a=await v();t&&(t.style.display="none"),a&&a.role==="supervisor"?(l.id=a.userId,l.name=a.name||"المشرف",s&&(s.textContent=l.name),e&&(e.style.display="block"),await g(l.id),n&&n.addEventListener("click",async()=>{await f(),window.location.href="/crs"})):window.location.href="/crs"}async function g(t){const e=document.getElementById("projects-list"),s=document.getElementById("no-projects-message");try{const a=await(await fetch(`/api/projects?supervisorId=${t}`,{headers:u()})).json();if(a.length===0){s&&(s.style.display="block"),e&&(e.innerHTML="");return}s&&(s.style.display="none"),e&&(e.innerHTML=a.map(x).join("")),j()}catch(n){console.error("Error fetching projects:",n)}}function x(t){const e=t.supervisors?t.supervisors.map(s=>s.name).join(", "):"-";return`
      ...
      `}function j(){document.querySelectorAll(".project-group").forEach(e=>{const s=e.querySelector(".expense-card"),n=e.querySelector(".report-card");e.querySelector(".btn-toggle-expense")?.addEventListener("click",()=>{s.style.display=s.style.display==="none"?"block":"none",n.style.display="none"}),e.querySelector(".btn-toggle-report")?.addEventListener("click",()=>{n.style.display=n.style.display==="none"?"block":"none",s.style.display="none"}),e.querySelector(".btn-cancel-expense")?.addEventListener("click",()=>s.style.display="none"),e.querySelector(".btn-cancel-report")?.addEventListener("click",()=>n.style.display="none");const a=e.querySelector(".item-name-select"),m=e.querySelector(".other-item-name-group");a?.addEventListener("change",()=>{m.style.display=a.value==="اخري"?"block":"none"}),e.querySelector(".expense-form")?.addEventListener("submit",async i=>{i.preventDefault();const o=i.target,r=o.querySelector(".btn-submit"),p=o.dataset.projectId;let c=o.querySelector(".item-name-select").value;c==="اخري"&&(c=o.querySelector(".other-item-name").value);const d=parseFloat(o.querySelector(".item-amount").value),y=o.querySelector(".expense-notes").value;r.disabled=!0,r.textContent="جارٍ الحفظ...";try{(await fetch("/api/dashboard/expenses",{method:"POST",headers:u({"Content-Type":"application/json"}),body:JSON.stringify({projectId:p,supervisorName:l.name,items:[{name:c,amount:d}],totalAmount:d,notes:y})})).ok&&(alert("تم الحفظ بنجاح"),o.reset(),s.style.display="none")}catch{alert("خطأ في الاتصال")}finally{r.disabled=!1,r.textContent="حفظ البند"}}),e.querySelector(".report-form")?.addEventListener("submit",async i=>{i.preventDefault();const o=i.target,r=o.querySelector(".btn-submit"),p=o.dataset.projectId,c=o.querySelector(".report-text").value;r.disabled=!0,r.textContent="جارٍ الحفظ...";try{(await fetch("/api/dashboard/reports",{method:"POST",headers:u({"Content-Type":"application/json"}),body:JSON.stringify({projectId:p,supervisorName:l.name,content:c})})).ok&&(alert("تم الحفظ بنجاح"),o.reset(),n.style.display="none")}catch{alert("خطأ في الاتصال")}finally{r.disabled=!1,r.textContent="حفظ التقرير"}})})}document.addEventListener("DOMContentLoaded",h);
